...
describe('isSubmittedToSii', () => {
  ...
  it('considera despachado un documento con Track ID o estado posterior a PENDING', () => {
    ...
    expect(isSubmittedToSii({ siiTrackId: null, siiStatus: 'ACCEPTED_WITH_OBJECTIONS' })).toBe(true);
  });

  it('no considera vigente un DTE rechazado por el SII, aunque tenga Track ID', () => {
    expect(isSubmittedToSii({ siiTrackId: '0123456789', siiStatus: 'REJECTED' })).toBe(false);
  });
});
...
describe('cancelSalesDocument — guard de envío al SII', () => {
  it('anula un DTE timbrado que aún no tiene Track ID', async () => {
    ...
    expect(updates).toContainEqual({
      where: { id: 'doc_1', companyId: 'cmp_1', status: 'ISSUED' },
  ...
  it('anula un DTE rechazado por el SII para poder volver a emitirlo', async () => {
    const document = docStub({ siiTrackId: '0123456789', siiStatus: 'REJECTED' });
    mockCancelTransaction(document);

    const result = await cancelSalesDocument('cmp_1', 'doc_1', 'Rechazado por el SII');
