...
describe('roles de agentes visibles', () => {
  it('los de eventos van con Certámenes y el equipo ejecutivo con CRM', () => {
    expect(visibleAgentRoles({ hasCrm: false, hasEventProjects: true })).toEqual(['EVENT_FINANCE', 'EVENT_COLLECTIONS']);
    expect(visibleAgentRoles({ hasCrm: true, hasEventProjects: false })).toEqual(['CEO', 'CFO', 'COO', 'SALES']);
    expect(visibleAgentRoles({ hasCrm: true, hasEventProjects: true })).toHaveLength(6);
  });
});
