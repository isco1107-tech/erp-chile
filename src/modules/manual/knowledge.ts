...
export const NAVIGATION_MAP: NavigationEntry[] = [
  { label: 'Dashboard', route: '/dashboard', group: 'Principal', purpose: 'Resumen general del negocio al entrar, con la guía de primeros pasos.' },
  { label: 'Pendientes', route: '/dashboard/inbox', group: 'Principal', anyOfPermissions: ['treasury:read', 'purchases:approve', 'expenses:approve', 'expenses:reimburse', 'leave:approve', 'payroll:read', 'contracts:read', 'bank:reconcile', 'paymentplans:read', 'sales:write'], purpose: 'Todo lo que requiere acción en la empresa, ordenado por urgencia: aprobaciones, cobros vencidos, borradores por emitir, cartola por conciliar, cotizaciones por pagar.' },
  ...
  { label: 'Contratos Recurrentes', route: '/dashboard/contracts', group: 'Servicios', requires: 'hasServiceContracts', permission: 'contracts:read', purpose: 'Igualas, mantenciones, arriendos y suscripciones que se facturan solos cada período (en borrador o emitidos automáticamente).' },
  { label: 'Control de Horas', route: '/dashboard/timesheets', group: 'Servicios', requires: 'hasTimesheets', permission: 'timesheets:log', purpose: 'Registrar horas por cliente y proyecto y convertir las facturables en un borrador de factura.' },
  ...
  { label: 'Flujo de Caja', route: '/dashboard/treasury/cashflow', group: 'Finanzas', requires: 'hasTreasury', permission: 'treasury:read', purpose: 'Todo el dinero que entró y salió (ventas, compras, sueldos, honorarios, rendiciones, cuotas, entradas…) con su origen, por medio de pago y por caja o banco.' },
  { label: 'Cajas & Bancos', route: '/dashboard/treasury/accounts', group: 'Finanzas', requires: 'hasTreasury', permission: 'treasury:read', purpose: 'Cajas y cuentas bancarias de la empresa con su saldo al día; la cuenta por defecto de cada tipo recibe los cobros y pagos que no indiquen otra.' },
  { label: 'Conciliación Bancaria', route: '/dashboard/treasury/reconciliation', group: 'Finanzas', requires: 'hasBankReconciliation', permission: 'bank:reconcile', purpose: 'Importar la cartola del banco (Excel/CSV) y enlazar cada movimiento con su cobro o pago, con sugerencias automáticas.' },
  ...
  { label: 'Cuentas del Sistema', route: '/dashboard/accounting/mappings', group: 'Contabilidad', requires: 'hasAccounting', permission: 'accounting:view', purpose: 'A qué cuenta del plan va cada asiento automático (sueldos, honorarios, cobros por documentar, gastos bancarios…).' },
  ...
  { label: 'Integraciones', route: '/dashboard/settings/integrations', group: 'Configuración', permission: 'settings:company', purpose: 'Estado de cada integración (SII, Khipu, cartolas, API, webhooks, calendario) y dónde se configura; aquí se conecta Khipu.' },
  { label: 'API REST', route: '/dashboard/settings/api', group: 'Configuración', requires: 'hasPublicApi', permission: 'api:manage', purpose: 'Crear y revocar llaves para que una tienda en línea u otra herramienta emita boletas y lea catálogo, stock y cobranza.' },
