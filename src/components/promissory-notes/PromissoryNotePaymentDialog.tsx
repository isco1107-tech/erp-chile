...
export default function PromissoryNotePaymentDialog({ noteId, amount, paidAmount }: Props) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const result = await registerPromissoryNotePaymentAction(noteId, { paidAmount: value });
      if (!result.success) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      ...
      <Button type="button" size="sm" disabled={saving || value > amount} onClick={handleSave}>
