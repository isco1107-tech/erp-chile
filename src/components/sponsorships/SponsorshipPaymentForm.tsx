...
import { PAYMENT_METHOD_TYPES, PAYMENT_METHOD_TYPE_LABELS } from '@/modules/sponsorships/schema';
...
export default function SponsorshipPaymentForm({ contractId, cashAmount, paidAmount }: Props) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const result = await updateSponsorshipPaymentAction(contractId, { paidAmount: amount, method });
      if (!result.success) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      ...
      {amount < paidAmount && <p className="text-xs text-destructive">El pago no puede ser menor al ya registrado</p>}
    </div>
  );
}
