...
import { buildPayrollWorkbook } from '@/modules/hr/services/payroll.service';

/**
 * Libro de remuneraciones del período en Excel. El proxy no intercepta /api:
...
export async function GET(_req: Request, { params }: { params: Promise<{ periodId: string }> }) {
  ...
  try {
    ...
    const { buffer, filename } = await buildPayrollWorkbook(session.companyId, periodId);
    ...
    return NextResponse.json({ success: false, error: 'No se pudo generar el libro de remuneraciones' }, { status: 500 });
  }
}
